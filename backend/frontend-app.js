...
async function loadProducts(cat = '', search = '', sort = '') {
  try {
    ...
    if (data.success) renderGrid(data.products);
  } catch (err) {
    console.error('Could not load products:', err);
    // Fallback to local products if backend not running
    applyFiltersLocal();
  }
}
...
function applyFilters() {
  ...
  loadProducts(activeCat, search, sort);
}
...
function applyFiltersLocal() {
  ...
  let items = products.filter(p => {
    ...
    const filterMatch =
      (!activeFilters.has('new') || p.isNew) &&
      (!activeFilters.has('sale') || p.isSale) &&
      (!activeFilters.has('top') || p.isTop);
...
function renderGrid(items) {
  ...
  if (!items.length) {
    g.innerHTML = '<div style="grid-column:1/-1;padding:3rem;text-align:center;color:#aaa;font-size:14px">No products found.</div>';
    return;
  }
  g.innerHTML = items.map(p => `
    <div class="product-card">
      <div class="product-img" style="background:${p.bg || '#f9f9f9'}">
        <span>${p.emoji}</span>
        <div class="p-wish ${liked[p.id] ? 'on' : ''}" onclick="toggleLike(${p.id}, this)">
          ${liked[p.id] ? '♥' : '♡'}
        </div>
      ...
      <div class="bag-overlay">
        ...
        <div class="size-row">
          ${p.sizes.map(s => `<div class="size-chip" onclick="addToBag(${p.id}, '${s}', ${JSON.stringify(p).replace(/'/g, "&#39;")})">${s}</div>`).join('')}
        </div>
        <button class="add-bag-btn" onclick="addToBag(${p.id}, '${p.sizes[0]}', ${JSON.stringify(p).replace(/'/g, "&#39;")})">ADD TO BAG</button>
      </div>
    </div>
  `).join('');
}
...
// ===== CART =====
function addToBag(id, size, product) {
...
function renderCartDrawer() {
  ...
  if (!items.length) {
    el.innerHTML = `<div class="empty-cart"><i class="ti ti-shopping-bag"></i><p style="font-size:15px;font-weight:700;margin-bottom:6px">Your bag is empty</p><p style="font-size:13px">Add items to it now</p></div>`;
    footer.innerHTML = '';
    return;
  }
...
// ===== RAZORPAY CHECKOUT =====
async function proceedToCheckout(total) {
  if (!authToken) {
  ...
  try {
    ...
    const res = await fetch(`${API}/payment/create-order`, {
      ...
      body: JSON.stringify({ amount: total, cartItems, address: { city: 'Delhi' } })
    });
    const data = await res.json();
    if (!data.success) return showToast('Order creation failed');
    ...
    const options = {
      ...
      name: 'Drip Fashion',
      description: 'Your order from Drip',
...
function closeAuth(e) { if (e.target === document.getElementById('authModal')) closeAuthDirect(); }
function closeAuthDirect() { document.getElementById('authModal').classList.remove('open'); }
...
async function doLogin() {
  ...
  try {
    ...
    showToast('Server not connected. Run the backend first!');
  }
}
...
async function loadProfile() {
  try {
    ...
    const data = await res.json();
    if (data.success) { currentUser = data.user; updateNavForLoggedIn(); }
    else { authToken = null; localStorage.removeItem('drip_token'); }
  } catch (err) { console.log('Backend offline - running in demo mode'); }
}

function updateNavForLoggedIn() {
  const profileIcon = document.querySelector('.nav-icon');
  if (profileIcon && currentUser) {
    profileIcon.innerHTML = `<i class="ti ti-user-check"></i><span>${currentUser.name.split(' ')[0]}</span>`;
  }
}
